...
public class ValidateUtil {
	...
	public static boolean isIdentityCard(EditText view) {
		...
		if (!matcher.find() && !matcher2.find()) {
			...
			if (licenc.length() != 15 && licenc.length() != 18) {
				view.setError("身份证号必须为15或18位数字（最后一位可以为X）");
				flag = false;
			}
			// 判断出生地
	 ...
	 * @return [0-9]{5,9}
	 */
	public static boolean isNO(String vins) {
		boolean flag = false;
		try {
			Pattern p = Pattern.compile("^[\u4e00-\u9fa5_a-zA-Z0-9]{17}$");
			Matcher m = p.matcher(vins);
			flag = m.matches();
		} catch (Exception e) {
			flag = false;
		}
	...
	// 验证车牌号
	public static boolean isCheNo(String cheString) {
		Pattern p = Pattern
				.compile("^[\u4e00-\u9fa5]{1}[a-zA-Z]{1}[a-zA-Z_0-9]{4}[a-zA-Z_0-9_\u4e00-\u9fa5]$");
		Matcher m = p.matcher(cheString);
		return m.matches();
	}
	...
	public static boolean isChepaihao(String string) {
