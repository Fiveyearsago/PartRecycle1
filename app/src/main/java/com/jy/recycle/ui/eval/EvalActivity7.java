 ...
 *
 */
public class EvalActivity7 extends JyBaseActivity  implements DialogUtil.DialogCallBack {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.eval_main7);
		context=this;
		...
		initLocation();
		EvalApplication.getInstance().addActivity(EvalActivity7.this);
		initBaseData();
	...
	private void getCityInfo() {
		new AsyncTask<Void, Void, JSONObject>() {
			...
			protected void onPreExecute() {
				progressDialog = ProgressDialog.show(EvalActivity7.this,
						"请稍候", "正在加载中...");
			}
	...
	private void findViews() {
		...
		this.personAdd.setOnTouchListener(new View.OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction() == MotionEvent.ACTION_DOWN) {
					Intent intent = new Intent(EvalActivity7.this, SelectAddressActivity.class);
		...
		contentPersonTxt.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				EvalActivity7.this.startActivityForResult(intent, 3);
			}
		});
				...
				if (checkPics()) {
					...
					if (mState.equals("1003")
						...
						for (PJInfo part : pjInfos) {
							...
							if (!erWeima.equals("") && erWeima != null) {
								flag = true;
							}
						}
						if (flag) {
							DialogUtil.showPosDialog(context, "未对数据进行修改，无需上传!", PublicStrings.REQUEST_CODE_UPLOAD_NO_COMMIT, EvalActivity7.this);
						}
					} else {
						// Log.i("tInfos.size()", tInfos.size() + "");
						if (tInfos != null && tInfos.size() == 0) {
							DialogUtil.showDialog(context, "是否上传数据，数据将为暂存状态!", PublicStrings.REQUEST_CODE_UPLOAD_COMMIT, EvalActivity7.this);
						} else {
							Intent i = new Intent(EvalActivity7.this,
...
//			}
	...
	private void partDialog(EvalPartInfo eInfo) {
		...
		mButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(EvalActivity7.this, ZBarCaptureActivity.class);
		...
		dialogOk.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (!"".equals(saomiaoText)
					...
					DialogUtil.showPosDialog(context, "已扫描过此零件，不能进行二次扫描!", PublicStrings.REQUEST_CODE_QRCODE_REPEAT, EvalActivity7.this);
				}
				dialog.dismiss();
				String ljbz = "";
	...
	private boolean checkPics() {
		if (reginstNo1.getText().length() < 4) {
			Toast.makeText(getApplicationContext(), "报案号长度是4位", Toast.LENGTH_SHORT).show();
			reginstNo1.requestFocus();
			Selection.setSelection((Spannable) reginstNo1.getText(), reginstNo1
					.getText().length());
			return false;
		}
		if (reginstNo2.getText().length() < 1) {
		...
		if (reginstNo3.getText().length() < 10) {
			Toast.makeText(getApplicationContext(), "报案号长度是10位", Toast.LENGTH_SHORT).show();
...
//		radioRepairYes.setEnabled(false);
	...
	private void bindHsdData() {
		...
		if (!mHsdInfo.getBah().equals("null") && !mHsdInfo.getBah().equals("")) {
			try {
				eInfo.setReportOne(mHsdInfo.getBah().substring(0, 4));
				eInfo.setReportTwo(mHsdInfo.getBah().substring(4, 5));
				eInfo.setReportThree(mHsdInfo.getBah().substring(5, 15));
				eInfo.setReportFour(mHsdInfo.getBah().substring(15, 21));
			} catch (Exception e) {
				// TODO: handle exception
				e.printStackTrace();
			}
