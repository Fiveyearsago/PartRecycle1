 ...
 *
 */
public class EvalActivity3 extends JyBaseActivity  implements DialogUtil.DialogCallBack {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.eval_main3);
		context=this;
		...
		EvalApplication.getInstance().addActivity(EvalActivity3.this);
		initBaseData();
...
//					});
	...
	private void findViews() {
		...
		this.personAdd.setOnTouchListener(new View.OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction() == MotionEvent.ACTION_DOWN) {
					Intent intent = new Intent(EvalActivity3.this, SelectAddressActivity.class);
		...
		contentPersonTxt.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				EvalActivity3.this.startActivityForResult(intent, 3);
			}
		});
			...
			public void onClick(View v) {
				...
				if (checkPics()) {
					...
					if (mState.equals("1003")
						...
						if (flag) {
							DialogUtil.showPosDialog(context, "未对数据进行修改，无需上传!", PublicStrings.REQUEST_CODE_UPLOAD_NO_COMMIT, EvalActivity3.this);
						}
					} else {
						// Log.i("tInfos.size()", tInfos.size() + "");
						if (tInfos != null && tInfos.size() == 0) {
							DialogUtil.showDialog(context, "是否上传数据，数据将为暂存状态!", PublicStrings.REQUEST_CODE_UPLOAD_COMMIT, EvalActivity3.this);
						} else {
							Intent i = new Intent(EvalActivity3.this,
	...
	private void partDialog(EvalPartInfo eInfo) {
		...
		if (mState.equals("1003") && evalPart.getOldDetail().equals("0")) {
			layout = LayoutInflater.from(context).inflate(
					R.layout.dag_editpart3_disable, null);
		} else {
			layout = LayoutInflater.from(context).inflate(
					R.layout.dag_editpart3, null);
		}
		...
		dialogOk.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (!"".equals(saomiaoText)
					...
					DialogUtil.showPosDialog(context, "已扫描过此零件，不能进行二次扫描!", PublicStrings.REQUEST_CODE_QRCODE_REPEAT, EvalActivity3.this);
				}
				dialog.dismiss();
				String ljbz;
	...
	private boolean checkPics() {
		...
		if (reginstNo3.getText().length() < 2) {
			...
			Selection.setSelection(reginstNo3.getText(), reginstNo3
					.getText().length());
			return false;
		}
		if (reginstNo4.getText().length() < 6) {
			...
			Selection.setSelection(reginstNo4.getText(), reginstNo4
					.getText().length());
			return false;
		}
	...
	private void delOnePart(final String remId, final String goodListId) {
		new AsyncTask<Void, Void, JSONObject>() {
			...
			protected JSONObject doInBackground(Void... params) {
				...
				return ServerApiManager.delPart(share.data()
